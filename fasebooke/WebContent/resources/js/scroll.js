...
window.description = "2222";
window.title="22222";
$(function(){
	var post = false;
	console.log($(window).scrollTop()+"这个方法是当前滚动条滚动的距离");
...
//		console.log(page.offsetHeight);
	    var page = $(".col-sm-7")[0];
	    if($("#main").scrollTop() >= page.offsetHeight - document.body.offsetHeight){
	    	if(post)
	    		return;
	    	post = true;
	    	...
	    	var l=$(".panel .panel-default").length;
	    		...
	    		$.ajax({
		    		 url:"/fasebooke/user/"+fuid+"/addmore.action",
		    	        type:"POST",
		    	        data:"offset="+l,
		    	        dataType:"json",
		    	        success:function(message){
		    	           ...
		    	           for(var i=0;i<neirong.length;i++){
			    	        	...
			    	        	if(neirong[i].imagehead == "")
			    	           	...
			    	           	description=neirong[i].sendwords;
			    	           	title=neirong[i].fname;
