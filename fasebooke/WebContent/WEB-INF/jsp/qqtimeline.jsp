 ...
 $('#imagewrap').css("background-size","cover");
    ...
    <%--  <img class="pic" src="http://localhost:8080/img/${current_user.fuid}/${current_user.fheadimage}">
     <span style="color:white;font-size:30px;">${current_user.fname}</span> --%>
       <c:choose>
                             <c:when test="${!empty current_user.fheadimage}">
                             <img class="pic" src="http://localhost:8080/img/${current_user.fuid}/${current_user.fheadimage}"><span style="color:white;font-size:30px;">${current_user.fname}</span>
                            </c:when>
                            <c:otherwise>
                             <img class="pic" src="http://localhost:8080/img/face/head_male_man_user_64px_1578_easyicon.net.png" ><span style="color:white;font-size:30px;">${current_user.fname}</span>
                            </c:otherwise>
                            </c:choose>
 </div>
						...
						<script>
                         ...
                         jiexi(sendwords,dataid,'');
								              ...
								              <%out.print("<span style='font-size:2px;'>"); %> ${comment.commetTime}<%out.print("</span>"); %>
								                  ...
								                  <%out.print("<a href='javascript:;' style='float:right' onclick='huifu(this)' data='"); %>${comment.id}<%out.print("' parent-data='");%>${obj.did}<%out.print("'>回复</a>");%>
      <c:if test="${!empty comment.replyComment}">
                         ...
                         jiexi(sendwords,dataid,'');
                                        ...
                                        <%out.print("<span style='font-size:2px;'>"); %> ${reply.commetTime}<%out.print("</span>"); %>
                                          <%out.print("<a href='javascript:;' style='float:right' onclick='huifu(this)' data='"); %>${reply.id}<%out.print("' parent-data='");%>${obj.did}<%out.print("'>回复</a></div>"); %>
   </c:forEach>   
   </c:if> 
   <%out.print("</div>");%>
			 					            </c:forEach>   
